...
namespace chenyi
{
    ...
    public class DefaultGenSphereStrategy : GenDetectionObjectStrategy
    {
        ...
        public int GetTriangleIndex(Vector3 point, Vector3 offset)
        {
            ...
            return GetTriangleIndex(Mathf.Atan(newPoint.z / Mathf.Sqrt(Mathf.Pow(newPoint.x, 2) + Mathf.Pow(newPoint.y, 2))),
                RadMap180To360(newPoint.x, newPoint.y));
        }
        public int GetTriangleIndex(float angleA, float angleB)
        {
            int index1 = GetVerticalAngleIndex(angleA);
            int index2 = GetHorizontalAngleIndex(angleB);
            if (index1 == 0)
            {
                return index2;
            }
            if (index1 == info.segmentVerticalCount - 1)
            {
                return info.segmentCircleCount * (1 + (info.segmentVerticalCount - 2) * info.segmentCircleCount * 2 + index2);
            }
            float x = Mathf.Cos(angleA) * Mathf.Cos(angleB);
            float y = Mathf.Sin(angleA);
            float z = Mathf.Cos(angleA) * Mathf.Sin(angleB);
            int pIndex1 = info.segmentCircleCount + (index1 - 1) * info.segmentCircleCount * 2 + index2 * 2;
            int pIndex2 = info.segmentCircleCount + 1 + (index1 - 1) * info.segmentCircleCount * 2 + index2 * 2 + 1;
            Vector3 line1 = new Vector3(x, y, z) - verticis[triangles[pIndex1 * 3 + 2]];
            Vector3 line2 = verticis[triangles[pIndex2 * 3 + 2]] - verticis[triangles[pIndex1 * 3 + 2]];//两个三角形的共边
            Vector3 line3 = verticis[triangles[pIndex1 * 3 + 2]] - verticis[triangles[pIndex1 * 3 + 1]];
        ...
        private float RadMap180To360(float x, float y)
        {
            if (x == 0)
            {
                if (y > 0)
